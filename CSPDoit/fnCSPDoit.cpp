...
#define CSP_OPT_SHOULD_FAILED(result) do {\
	if (!result) {\
		return GetLastError();\
	}\
}while(0)
...
CSPDOIT_API DWORD fnGenP10BySoft(IN char* pData, IN char* keyBitsLen, OUT char* pP10Base64Data, OUT long* lP10Base64Datalen)
{
	...
	*lP10Base64Datalen = 0;
	...
	OpenSSL_add_all_algorithms();	// must need it
									//test code
	EVP_PKEY_ptr userPubKey(EVP_PKEY_new(), EVP_PKEY_free);
	BN_ptr e1(BN_new(), BN_free);
	RSA_ptr rsa1(RSA_new(), RSA_free);
	BN_set_word(e1.get(), RSA_F4);
	RSA_generate_key_ex(rsa1.get(), atol(keyBitsLen), e1.get(), nullptr);
	EVP_PKEY_set1_RSA(userPubKey.get(), rsa1.get());
	X509_REQ_set_pubkey(x509Req.get(), userPubKey.get());

	if (0 != SavePrivateKey(userPubKey.get(), nullptr))
	{
		return 5;
	}

	X509_REQ_sign(x509Req.get(), userPubKey.get(), EVP_sha256());
